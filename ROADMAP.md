...
- **[Issue #335](https://github.com/hgourvest/node-firebird/issues/335)** — BLOB loading slowly ✅ Closed (resolved by `blobChunkSize`/`blobReadChunkSize`); documented in [README.md § FAQ](README.md#faq)
...
- **[Issue #332](https://github.com/hgourvest/node-firebird/issues/332)** — LIKE clause error in SELECT ✅ Closed (server-side DSQL parameter-length behavior, not a driver bug; not reproducible on Firebird 6.0); workaround documented in [README.md § FAQ](README.md#faq)
...
13. **Exact numeric results (`numericMode`)** ✅ Implemented (2.15.0) — opt-in connection/URI option for `BIGINT`, `INT128` and the `NUMERIC`/`DECIMAL` types backed by them: `lossy` (default, unchanged behaviour), `safe` (number when the raw coefficient is within `Number.MAX_SAFE_INTEGER`, otherwise an exact scaled string) or `string` (always an exact, scale-preserving string). Fixes precision loss that `typeCast` cannot recover after decoding. Constants `NUMERIC_MODE_LOSSY`/`SAFE`/`STRING` and the `NumericMode` type are exported. Documented in [README.md § Fixed-point numeric results](README.md#fixed-point-numeric-results-numericmode).
...
12. **Multi-host pooling (`PoolCluster`)** ✅ Implemented — `Firebird.poolCluster`: named nodes (shared `defaults` + per-node overrides), each backed by a regular pool (health checks, recycling, metrics per node), selected by glob pattern with `rr`/`random`/`order` selectors. Connection failures fail over to the next matching online node; `removeNodeErrorCount` consecutive failures take a node offline (`'offline'` event) with `restoreNodeTimeout` timed restoration or manual `restore()`. `of(pattern)` facades, `withConnection`, `status()` metrics, dynamic `add`/`remove`, `destroy`. Documented in [README.md § Multi-host pooling](README.md#multi-host-pooling-poolcluster); live tests in `test/pool-cluster.js`. This was the LAST open roadmap item.
...
13. **Affected-rows count + result metadata in the promise API** ✅ Implemented — per-query `withMeta: true` (callback and promise APIs) delivers `{ rows, fields, affectedRows, recordCounts, warnings }` without touching the bare-rows default. For DML — including `RETURNING` and `EXECUTE PROCEDURE` — `affectedRows` is the server-reported count (`isc_info_sql_records` via a new `op_info_sql` request, issued while the statement handle is still open) with per-verb `recordCounts`; for SELECT it is `rows.length` (pg's `rowCount` convention, no extra round-trip). `fields` reuses the typeCast column vocabulary plus nullability and relation alias/schema; TypeScript overloads resolve `queryAsync<T>(…, { withMeta: true })` to `QueryResult<T>`. Documented in [README.md § Result metadata and affected rows](README.md#result-metadata-and-affected-rows-withmeta); tests in `test/query-meta.js`.
14. **Tagged-template query API** ✅ Implemented — `db.sql` / `tx.sql` (Postgres.js style): interpolations compile to positional `?` params client-side (injection-safe by construction), queries are lazy thenables executing exactly once on await, embedded queries compose as fragments (text + params spliced in place), `sql('NAME')` produces safely quoted dot-qualified identifiers, arrays expand to `?, ?, ?` lists for `IN`, and `.options()` / `.withMeta()` / `.toQuery()` tie into the existing per-query options, the #13 result shape, and testing/logging. Pure client-side sugar in `src/sql-template.ts` over `queryAsync` — the callback API is untouched. Documented in [README.md § Tagged-template queries](README.md#tagged-template-queries-sql); tests in `test/sql-template.js` + `test/unit/sql-template.test.ts`. (Insert-from-object builders deferred until asked for.)
15. **Surface server warnings** ✅ Implemented — every `isc_arg_warning` on a successful response is now emitted as a `warning` driver event (`{ gdscode, params, message }`, message resolved via firebird.msg with a gdscode+params fallback for codes newer than the bundled file) and included in `withMeta` results. Emission is deferred one tick so a listener registered inside the attach callback still catches attach-time warnings (e.g. "parallel workers value capped"). Documented in [README.md § Driver Events](README.md#driver-events); test in `test/query-meta.js`.
16. **Savepoint helpers** ✅ Implemented — `transaction.savepoint(work)`: SAVEPOINT on entry, RELEASE on resolve, ROLLBACK TO on reject (undoing only work's changes, transaction stays usable, original error rethrown with any rollback failure attached as `err.savepointRollbackError`). Nestable via generated names, mirroring `withTransaction` / Postgres.js `sql.savepoint()`. Documented in [README.md § Savepoints](README.md#savepoints); live tests in `test/savepoints.js`.
17. **Environment-variable configuration defaults** ✅ Implemented — `normalizeOptions` (the single entry point for attach/create/pool/URI strings) falls back to `ISC_USER` / `ISC_PASSWORD` (Firebird's own isql conventions) plus `FIREBIRD_HOST` / `FIREBIRD_PORT` / `FIREBIRD_DATABASE` / `FIREBIRD_ROLE` for settings the caller omitted. Explicit options always win, built-in defaults apply last, caller-owned option objects are never mutated. Documented in [README.md § Connection options](README.md#connection-options); unit tests in `test/unit/env-defaults.test.ts`.
18. **Pool connection recycling** ✅ Implemented — `maxUses` (pg) retires a physical connection after N checkouts, `maxLifetimeMillis` (Postgres.js `max_lifetime`) T ms after creation. Enforced when the connection returns to the pool, and the idle sweep also retires over-lifetime idle connections (even below `min` — recycling is the point; replacements are created on demand). Both default off. Documented in [README.md § Pool events and metrics](README.md#pool-events-and-metrics); live tests in `test/pool-recycling.js`.
19. **Row-key transforms** ✅ Implemented — `transformKeys` (connection-level and per-query): the built-in `'camel'` (`FIRST_NAME` → `firstName`) or a custom `(key) => key` mapper, applied in `computeColumnKeys` after `lowercase_keys` and to both parts of `nestTables` keys, so the decoder and the blob resolver stay in agreement. Custom mappers are guarded like typeCast (a throw falls back to the untransformed key instead of desyncing the decode loop). Column metadata keeps raw aliases. Documented in [README.md § Transforming row keys](README.md#transforming-row-keys-transformkeys); live tests in `test/transform-keys.js`.
20. **Bulk-insert Writable stream** ✅ Implemented — `db.batchStream(sql, { flushRows })` / `transaction.batchStream(...)`: an object-mode Writable flushing parameter rows in chunks through one prepared statement via the Firebird 4 batch API, with Writable backpressure (writes pause while a chunk is in flight). The Database form is all-or-nothing in its own transaction (commit on finish, rollback on error/destroy); totals in `stream.recordCount`/`affectedRows`. The BLOB prerequisite landed with it: `executeBatch` now accepts Buffers/strings/objects on BLOB columns — uploaded as transaction blobs (initiated back-to-back so the deferred blob ops pipeline) and referenced by quad id in the batch messages (default BLOB_NONE policy). ARRAY params remain unsupported. Documented in [README.md § Bulk-insert stream](README.md#bulk-insert-stream-batchstream-firebird-40); tests in `test/batch-stream.js` (protocol-gated). |
