...
| Next minor | Result metadata + affected rows (`withMeta`, #13) ✅ done; server `warning` events (#15) ✅ done; tagged-template query API (#14) ✅ done |
| Later minors | Remaining Round 2 gaps, by impact: savepoint helpers (#16); env-var config defaults (#17); pool `maxUses`/`maxLifetime` recycling (#18); row-key transforms (#19); bulk-insert Writable stream (#20) |
