...
| Next minor | Result metadata + affected rows (`withMeta`, #13) ✅ done; server `warning` events (#15) ✅ done; tagged-template query API (#14) ✅ done; savepoint helpers (#16) ✅ done; env-var config defaults (#17) ✅ done |
| Later minors | Remaining Round 2 gaps: pool `maxUses`/`maxLifetime` recycling (#18); row-key transforms (#19); bulk-insert Writable stream (#20) |
