...
exports.serverSeed = function(user, password, salt, b, hashAlgo = 'sha1') {
    ...
    var k = BigInt('0x' + getHash(hashAlgo, pad(PRIME.N), pad(PRIME.g)));
    var kv = (k * v) % PRIME.N;
    var B = (kv + gb) % PRIME.N;
...
exports.clientProof = function(user, password, salt, A, B, a, hashAlgo = 'sha1') {
    ...
    n1 = toBigInt(getHash(hashAlgo, toBuffer(PRIME.N)));
    n2 = toBigInt(getHash(hashAlgo, toBuffer(PRIME.g)));

    dump('n1', n1);
    dump('n2', n2);

    n1 = modPow(n1, n2, PRIME.N);
    n2 = toBigInt(getHash(hashAlgo, user));
...
function clientSession(user, password, salt, A, B, a, hashAlgo = 'sha1') {
    ...
    var K = toBigInt(getHash(hashAlgo, toBuffer(sessionSecret)));
