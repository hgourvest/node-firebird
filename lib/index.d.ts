...
declare module 'node-firebird' {
  ...
  export interface Database {
    ...
    sequentially(
      query: string,
      params: any[],
      rowCallback: SequentialCallback,
  ...
  export type CharsetSetSerialize =
    | 'ascii'
    | 'utf8'
    | 'utf-8'
    | 'utf16le'
  ...
  export interface Options {
    ...
    encoding_serialize: CharsetSetSerialize;
  }
  ...
  export function attach(
    options: SvcMgrOptions,
    callback: ServiceManagerCallback
  ): void;
  export function escape(
    value: any,
    protocolVersion?: number /*PROTOCOL_VERSION13*/
  ): string;
  ...
  type LineCallback = (
    err: any,
    data: { result: number; line: string }
  ) => void;
  ...
  export interface ServiceManager {
    ...
    getFbserverInfos(
      infos: ServerInfoReq,
      options: { buffersize?: number; timeout?: number },
      callback: InfoCallback
    ): void; // if infos is empty all options are asked to the service
