...
SQLVarText.prototype.decode = function(data, lowerV13, charset) {
    let ret;
    if (this.subType > 1) {
        // ToDo: with column charset
        ret = data.readText(this.length, charset);
...
SQLVarString.prototype.decode = function(data, lowerV13, charset) {
    let ret;
    if (this.subType > 1) {
        // ToDo: with column charset
        ret = data.readString(charset);
    } else if (this.subType === 0) {
        // without charset definition
        ret = data.readString(charset);
    } else {
...
function SQLParamString(value, charset = Const.DEFAULT_ENCODING) {
    this.value = value;
...
SQLParamString.prototype.encode = function(data) {
    if (this.value != null) {
        data.addText(this.value, this._charset);
    } else {
        data.addInt(1);
    }
};
...
SQLParamString.prototype.calcBlr = function(blr) {
    blr.addByte(Const.blr_text);
    var len = this.value ? Buffer.byteLength(this.value, this._charset) : 0;
    blr.addWord(len);
};
