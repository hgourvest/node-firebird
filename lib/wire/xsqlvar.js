...
class SQLVarTimeTz {
    decode(data, lowerV13) {
        var ret = data.readTimeTz();

        if (!lowerV13 || !data.readInt()) {
            var d = new Date(0);
            d.setMilliseconds(Math.floor(ret.time / 10));
            d.timeZoneId = ret.tz;
            return d;
        }
        return null;
...
class SQLVarTimeStampTz {
    decode(data, lowerV13) {
        var ret = data.readTimeStampTz();

        if (!lowerV13 || !data.readInt()) {
            var ms = (ret.date - DateOffset) * TimeCoeff + Math.floor(ret.time / 10);
            var d = new Date(ms);
            d.timeZoneId = ret.tz;
            return d;
        }
    ...
    calcBlr(blr) {
        blr.addByte(Const.blr_sql_timestamp_tz);
    }
}

//------------------------------------------------------

class SQLVarTimeTzEx {
    decode(data, lowerV13) {
        var ret = data.readTimeTzEx();

        if (!lowerV13 || !data.readInt()) {
            var d = new Date(Math.floor(ret.time / 10));
            d.timeZoneId = ret.tz;
            d.timeZoneOffset = ret.offset;
            return d;
        }
        return null;
    }
...
class SQLVarTimeStampTzEx {
    decode(data, lowerV13) {
        var ret = data.readTimeStampTzEx();
...
class SQLParamTimeTz {
    ...
    encode(data) {
        if (this.value != null) {
            var dateValue = this.value instanceof Date ? this.value : this.value.value;
            var tz = this.value.timeZoneId || 65535; // Default to UTC/GMT

            var ms = dateValue.getTime();
            var time = (ms % TimeCoeff) * 10;

            data.addTimeTz(time, tz);
        } else {
            data.addTimeTz(0, 0);
            data.addInt(1);
        }
...
class SQLParamTimeStampTz {
    ...
    encode(data) {
        if (this.value != null) {
            var dateValue = this.value instanceof Date ? this.value : this.value.value;
            var tz = this.value.timeZoneId || 65535;
            ...
            data.addTimeStampTz(date, time, tz);
        } else {
            data.addTimeStampTz(0, 0, 0);
            data.addInt(1);
        }
    }
