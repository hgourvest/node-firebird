...
class FbEventManager extends Events.EventEmitter {
    ...
    close(callback) {
        ...
        // Wait for the event socket to fully close before invoking the callback.
        // This prevents a race condition on Firebird 4 where the server is still
        // processing the previous event connection's FIN when the next queEvents
        // arrives on the main connection, causing the op_response to be dropped.
        function endAndWaitForClose(cb) {
            var sock = self.eventconnection._socket;
            if (sock.destroyed) {
                if (cb) cb();
                return;
            }
            ...
            sock.end();
            // Safety fallback: if 'close' never fires (e.g. Firebird does not
            // send its FIN), resolve after 200 ms so tests don't hang.
            setTimeout(done, 200);
        }
