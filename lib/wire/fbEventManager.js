...
//   │  waiting for op_response │    │  sock.destroy()        │
//   └──────────┬───────────────┘    └───────────┬───────────┘
...
class FbEventManager extends Events.EventEmitter {
    ...
    close(callback) {
        ...
        // Immediately destroy the event socket and wait for the 'close' event
        // before invoking the callback.  Using destroy() (RST) rather than
        // end() (FIN) ensures that Firebird processes the teardown instantly:
        // with end() the server may take >200 ms to echo its own FIN, leaving
        // the previous EventConnection in a half-closed state that races with
        // the very next op_que_events on the main connection and causes
        // Firebird to silently drop the op_response.  RST avoids that window.
        function endAndWaitForClose(cb) {
            ...
            sock.destroy();
            // Safety fallback: if 'close' never fires for any reason.
