                ...
                statement.connection.openBlob(id, transaction, (err, blob) => {
                    ...
                    const read = () => {
                        statement.connection.getSegment(blob, (err, ret) => {
                            ...
                            // Send op_close_blob immediately (not deferred) so the
                            // server can free the blob handle and keep processing
                            // getSegment requests for other concurrently open blobs.
                            // Deferring close_blob with many concurrent blobs causes
                            // a deadlock: the server waits for close_blob before
                            // sending more data, but the client never flushes it.
                            // See: https://github.com/hgourvest/node-firebird/issues/387
                            statement.connection.closeBlob(blob, undefined, false);
                            if (singleTransaction) {
        ...
        statement.connection.openBlob(id, transaction, (err, blob) => {
            ...
            const read = () => {
                statement.connection.getSegment(blob, (err, ret) => {
                    ...
                    // Same reason as fetch_blob_async_transaction: send
                    // op_close_blob immediately to prevent server deadlock.
                    // See: https://github.com/hgourvest/node-firebird/issues/387
                    statement.connection.closeBlob(blob, undefined, false);
                    if (close) {
                        transaction.commit((err) => {
