...
class Connection {
    ...
    connect(options, callback) {
        ...
        function cb(err, ret) {
            if (err) {
                process.stderr.write('[DBG:connect] cb err: ' + (err && err.message) + ' plugin=' + (options.pluginName||'default') + '\n');
            ...
            self.accept = ret;
            process.stderr.write('[DBG:connect] auth ok, plugin=' + (options.pluginName||'default') + ' proto=' + (ret && ret.protocolVersion) + ' sessionKey=' + !!(ret && ret.sessionKey) + '\n');
            ...
            if (ret.sessionKey && ret.protocolVersion >= Const.PROTOCOL_VERSION13 &&
                ...
                process.stderr.write('[DBG:connect] sending op_crypt Arc4, plugin=' + (options.pluginName||'default') + '\n');
                self.sendOpCrypt('Arc4');
                self._socket.enableEncryption(keyBuf);
                self._pending.push('crypt');
                self._queue.push(function(cryptErr) {
                    if (cryptErr) {
                        process.stderr.write('[DBG:connect] crypt err: ' + (cryptErr && cryptErr.message) + '\n');
                        doError(cryptErr, callback);
                        return;
                    }
                    process.stderr.write('[DBG:connect] crypt ok, calling final callback, plugin=' + (options.pluginName||'default') + '\n');
                    if (callback)
...
function decodeResponse(data, callback, cnx, lowercase_keys, cb) {
    try {
        ...
        // Log connect-phase opcodes to help diagnose auth hangs
        if (cnx && cnx._pending && cnx._pending.length > 0 && (cnx._pending[0] === 'connect' || cnx._pending[0] === 'crypt')) {
            process.stderr.write('[DBG:decode] pending=' + cnx._pending[0] + ' opcode=' + r + ' plugin=' + (cnx.options && cnx.options.pluginName || 'default') + '\n');
        }
        ...
        switch (r) {
            ...
            case Const.op_accept_data:
                ...
                if (r === Const.op_cond_accept && accept.authData) {
                    process.stderr.write('[DBG:auth] op_cond_accept, sending op_cont_auth with plugin=' + accept.pluginName + ' clientPlugin=' + (cnx.options.pluginName||'default') + '\n');
                ...
                process.stderr.write('[DBG:auth] op_accept/op_accept_data, calling cb. r=' + r + ' plugin=' + accept.pluginName + '\n');
            ...
            case Const.op_cont_auth:
                ...
                process.stderr.write('[DBG:auth] SERVER sent op_cont_auth, server-plugin=' + pluginName + ' clientPlugin=' + (cnx.options && cnx.options.pluginName||'default') + ' pendingAccept=' + !!(cnx && cnx._pendingAccept) + '\n');

                if (!cnx.options.pluginName) {
                    if (cnx.accept.pluginName === pluginName) {
                        // Erreur plugin not able to connect
                        process.stderr.write('[DBG:auth] op_cont_auth: plugin same as accept.pluginName, returning error\n');
                ...
                // Log the buffer so we can diagnose in CI.
                process.stderr.write('[DBG:auth] op_cont_auth: unhandled server op_cont_auth, bufLen=' + (d && d.buffer && d.buffer.length) + ' returning without calling cb — CONNECTION WILL HANG\n');
