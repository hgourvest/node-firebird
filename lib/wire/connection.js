        ...
        switch (r) {
            ...
            case Const.op_cont_auth:
                ...
                if (cnx._pendingAccept && pluginName === Const.AUTH_PLUGIN_LEGACY) {
                    if (process.env.FIREBIRD_DEBUG) {
                        console.log('[fb-debug] op_cont_auth: SRP+Legacy_Auth chained-auth (proto %d), sending Legacy_Auth credentials',
                ...
                if (process.env.FIREBIRD_DEBUG) {
                    console.warn('[fb-debug] op_cont_auth: unhandled plugin=%s pendingAccept=%s options.plugin=%s',
                        pluginName,
