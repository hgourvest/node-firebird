...
class Connection {
                ...
                if (value === null || value === undefined) {
                    switch (meta.type) {
                        ...
                        case Const.SQL_TIMESTAMP:
                            ...
                            break;
                        case Const.SQL_TYPE_TIME_TZ:
                            ret[i] = new Xsql.SQLParamTimeTz(null);
                            break;
                        case Const.SQL_TYPE_TIMESTAMP_TZ:
                            ret[i] = new Xsql.SQLParamTimeStampTz(null);
                            break;
                        case Const.SQL_BLOB:
                    ...
                    switch (meta.type) {
                        ...
                        case Const.SQL_TYPE_TIME_TZ:
                        case Const.SQL_TYPE_TIMESTAMP_TZ:
                            var paramClass = Xsql.SQLParamDate;
                            if (meta.type === Const.SQL_TYPE_TIME_TZ) paramClass = Xsql.SQLParamTimeTz;
                            else if (meta.type === Const.SQL_TYPE_TIMESTAMP_TZ) paramClass = Xsql.SQLParamTimeStampTz;

                            if (value instanceof Date)
                                ret[i] = new paramClass(value);
                            else if (typeof(value) === 'string')
                                ret[i] = new paramClass(parseDate(value));
                            else
                                ret[i] = new paramClass(new Date(value));
    ...
    sendExecute(op, statement, transaction, callback, parameters) {
        ...
        if (parameters && parameters.length) {
            ...
            if (this.accept.protocolVersion >= Const.PROTOCOL_VERSION13) {
                ...
                if (nullBuffer.length) {
                    msg.addBytes(nullBuffer);
                }
                if (remainingBytes > 0) {
                    msg.addBytes(Buffer.alloc(remainingBytes));
                }
        ...
        if (this.accept.protocolVersion >= Const.PROTOCOL_VERSION16) {
            var timeout = statement.options?.timeout || 0;
            console.log('DEBUG: sendExecute protocol:', this.accept.protocolVersion, 'timeout:', timeout);
            msg.addInt(timeout); // p_sqldata_timeout
        }
    ...
    try {
        ...
        switch (r) {
            ...
            case Const.op_sql_response:
                ...
                if (isOpFetch) {
                    console.log('DEBUG: op_fetch_response status:', data.fstatus, 'count:', data.fcount);
                }
    ...
    while (br.pos < br.buffer.length) {
        ...
        switch (outerTag) {
            ...
            case Const.isc_info_sql_describe_vars:
                    ...
                    switch (innerTag) {
                        ...
                        case Const.isc_info_sql_type:
                            ...
                            switch (type&~1) {
                                ...
                                case Const.SQL_TYPE_TIME_TZ: param = new Xsql.SQLVarTimeTz(); break;
                                case Const.SQL_TYPE_TIMESTAMP_TZ: param = new Xsql.SQLVarTimeStampTz(); break;
                                ...
                                case Const.SQL_DEC34:     param = new Xsql.SQLVarDouble(); break;
                                case Const.SQL_BOOLEAN:   param = new Xsql.SQLVarBoolean(); break;
                                default:
                                    throw new Error('Unexpected SQL type: ' + (type & ~1));
                            }
                        ...
                        case Const.isc_info_sql_length:
                            ...
                            if (param.type === Const.SQL_TYPE_TIME_TZ && param.length === 8) {
                                Object.setPrototypeOf(param, Xsql.SQLVarTimeTzEx.prototype);
                            } else if (param.type === Const.SQL_TYPE_TIMESTAMP_TZ && param.length === 12) {
                                Object.setPrototypeOf(param, Xsql.SQLVarTimeStampTzEx.prototype);
                            }
                            break;
                ...
                break;
            default:
                // Unknown outer item - stop parsing to avoid infinite loop
                br.pos = br.buffer.length; 
                break;
        }
    }
}
