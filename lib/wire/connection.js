...
Connection.prototype._bind_events = function(host, port, callback) {
    ...
    self._socket.on('data', function (data) {
        ...
        while (xdr.pos < xdr.buffer.length) {
            ...
            decodeResponse(xdr, cb, self, self._lowercase_keys, self._charset, function (err, obj) {
...
function decodeResponse(data, callback, cnx, lowercase_keys, charset, cb) {
    try {
        do {
        ...
        switch (r) {
            case Const.op_response:
                    ...
                    if (err) {
                        ...
                        if (callback && callback.lazy_count) {
                            ...
                            if (callback.lazy_count > 0) {
                                ...
                                parseOpResponse(charset, data, response, loop);
                            } else {
                                cb(null, response);
                            }
                        } else {
                ...
                return parseOpResponse(charset, data, response, loop);
            ...
            case Const.op_sql_response:
                ...
                if (isOpFetch && data.fop) { // could be set when a packet is not complete
                    ...
                    if (op === Const.op_response) {
                        return parseOpResponse(charset, data, {}, cb);
                    }
                }
                ...
                while (data.fcount && (data.fstatus !== 100)) {
                    ...
                    for (let length = output.length; data.fcolumn < length; data.fcolumn++) {
                        ...
                        try {
                            ...
                            if ([Const.SQL_TEXT, Const.SQL_VARYING].includes(item.type)) {
                                value = item.decode(data, lowerV13, charset);
                            } else {
                            ...
                            if (item.type === Const.SQL_BLOB && value !== null) {
                                if (item.subType === Const.isc_blob_text && cnx.options.blobAsText) {
                                    value = fetch_blob_async_transaction(statement, value, key, row, charset);
                                    arrBlob.push(value);
                                } else {
...
function parseOpResponse(charset, data, response, cb) {
    ...
    while (true) {
        ...
        switch (op) {
            ...
            case Const.isc_arg_sql_state:
                if (item.params) {
                    var str = data.readString(charset);
                    item.params.push(str);
                } else {
                    item.params = [data.readString(charset)];
                }
...
function describe(buff, statement, charset) {
...
Connection.prototype.prepareStatement = function (transaction, statement, query, plan, callback) {
    ...
    this._queueEvent(function(err, ret) {
        ...
        if (!err) {
            describe(ret.buffer, statement, this._charset);
...
Connection.prototype.executeStatement = function(transaction, statement, params, callback, custom) {
    ...
    function PrepareParams(params, input, callback, charset) {
        ...
        function putBlobData(index, value, callback, charset) {
        ...
        for (var i = 0, length = params.length; i < length; i++) {
            ...
            if (value === null || value === undefined) {
                switch (meta.type) {
                    ...
                    case Const.SQL_NULL:
                    case Const.SQL_TEXT:
                        ret[i] = new Xsql.SQLParamString(null, charset);
                        break;
                ...
                switch (meta.type) {
                    case Const.SQL_BLOB:
                        putBlobData(i, value, done, charset);
                        break;
                    ...
                    default:
                        switch (typeof value) {
                            ...
                            case 'number':
                                ...
                                break;
                            case 'string':
                                ret[i] = new Xsql.SQLParamString(value, charset);
                                break;
                            case 'boolean':
                            ...
                            default:
                                ...
                                ret[i] = new Xsql.SQLParamString(value.toString(), charset);
                                break;
                        }
                        done();
                }
...
function fetch_blob_async_transaction(statement, id, column, row, charset) {
