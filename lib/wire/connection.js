...
class Connection {
    ...
    auxConnection(callback) {
        ...
        function cb(err, ret) {
            ...
            console.error('[DBG:cnx] auxConnection: response arrived port=%d host=%s queueLen=%d', socket_info.port, socket_info.host, self._queue.length);
            callback(undefined, socket_info);
        }
        console.error('[DBG:cnx] auxConnection: sending op_connect_request queueLen=%d', this._queue.length);
        this._queueEvent(cb);
    }
    ...
    queEvents(events, eventid, callback) {
        ...
        console.error('[DBG:cnx] queEvents: sending op_que_events eventid=%d events=%j queueLen=%d', eventid, events, this._queue.length);
        function cb(err, ret) {
            console.error('[DBG:cnx] queEvents: response arrived eventid=%d err=%s', eventid, err ? err.message : 'null');
            if (err) {
                doError(err, callback);
                return;
            }
    ...
    closeEvents(eventid, callback) {
        ...
        msg.addInt(self.dbhandle);
        msg.addInt(eventid);
        console.error('[DBG:cnx] closeEvents: sending op_cancel_events eventid=%d queueLen=%d', eventid, self._queue.length);
    
        function cb(err, ret) {
            console.error('[DBG:cnx] closeEvents: response arrived eventid=%d err=%s', eventid, err ? err.message : 'null');
            if (err) {
                doError(err, callback);
                return;
            }
    ...
    try {
        ...
        switch (r) {
            ...
            case Const.op_crypt_key_callback:
                ...
                return;
            case Const.op_event:
                // op_event can arrive on the main connection (e.g. after cancel_events on
                // some Firebird versions). Read and discard the event data so the buffer
                // stays in sync. Do NOT call cb or shift the queue – the queued callback
                // is waiting for the real op_response that follows.
                {
                    var evDbHandle = data.readInt();    // db handle (discarded)
                    var evEpb = data.readArray();       // EPB (event parameter block, discarded)
                    var evAst = data.readInt64();       // AST info (discarded)
                    var evEventId = data.readInt();     // event id (discarded)
                    console.error('[DBG:mainConn] op_event on MAIN conn: dbh=%d eventid=%d queueLen=%d', evDbHandle, evEventId, cnx ? cnx._queue.length : -1);
                }
                return;
            default:
