...
class Connection {
    ...
    connect(options, callback) {
        ...
        this.requestedPluginName = pluginName;
        var msg = this._msg;
        var blr = this._blr;
            ...
            case Const.op_accept_data:
                ...
                if (r === Const.op_cond_accept || r === Const.op_accept_data) {
                    ...
                    if (is_authenticated === 0) {
                        var reqPlugin = cnx.requestedPluginName || cnx.options.pluginName;
                        if (reqPlugin && reqPlugin !== accept.pluginName) {
                            var errPlugin = new Error('Server don\'t accept plugin : ' + reqPlugin + ', but support : ' + accept.pluginName);
