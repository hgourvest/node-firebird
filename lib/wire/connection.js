...
class Connection {
    ...
    _bind_events(host, port, callback) {
        ...
        self._socket.on('data', function (data) {
            ...
            if (process.env.FIREBIRD_DEBUG) {
                console.log('[fb-debug] data event: bytes=%d queue=%d xdr.pos=%d', xdr.buffer.length, self._queue.length, xdr.pos);
            }
                ...
                decodeResponse(xdr, cb, self, self._lowercase_keys, function (err, obj) {
                    ...
                    if (err) {
                        if (err instanceof RangeError) {
                            ...
                            // Protocol-level error (e.g. unexpected opcode).  Do NOT
                            // save the buffer – saving it causes infinite re-processing
                            // of the same garbage bytes, permanently blocking the queue.
                            // Instead, fail the pending callback so the queue can advance.
                            if (process.env.FIREBIRD_DEBUG) {
                                console.warn(`[fb-debug] protocol warning: ${err.message} pos=${pos} queue=${self._queue.length}`);
                            }
                            if (cb) {
                                self._queue.shift();
                                self._pending.shift();
                    ...
                    // op_event received on the main connection: data has been
                    // consumed by decodeResponse but it does not belong to any
                    // queued request – do NOT shift the queue or invoke any
                    // pending callback.
                    if (obj && obj._isOpEvent) {
                        if (process.env.FIREBIRD_DEBUG) {
                            console.log('[fb-debug] op_event on main connection (ignored), queue=%d', self._queue.length);
                        }
                        return;
                    }
                    ...
                    if (process.env.FIREBIRD_DEBUG) {
                        console.log('[fb-debug] response dispatched, queue remaining=%d', self._queue.length);
                    }
    ...
    auxConnection(callback) {
        ...
        msg.addInt(0);
        if (process.env.FIREBIRD_DEBUG) {
            console.log('[fb-debug] auxConnection: sending op_connect_request, dbhandle=%d queue_before=%d', self.dbhandle, self._queue.length);
        }
        function cb(err, ret) {
    
            if (err) {
                if (process.env.FIREBIRD_DEBUG) {
                    console.log('[fb-debug] auxConnection: error:', err.message);
                }
                doError(err, callback);
                return;
            }
            ...
            if (process.env.FIREBIRD_DEBUG) {
                console.log('[fb-debug] auxConnection: got socket_info family=%d port=%d host=%s', socket_info.family, socket_info.port, socket_info.host);
            }
    ...
    try {
        ...
        switch (r) {
            ...
            case Const.op_event:
                ...
                data.readInt();    // p_event_database
                data.readArray();  // p_event_items (EPB buffer)
                data.readInt64();  // p_event_ast
                data.readInt();    // p_event_rid
                return cb(null, { _isOpEvent: true });
            default:
                // Unknown or unhandled opcode.  Return an error so the data
                // handler can decide how to handle it (fail the pending
                // callback, log a warning, etc.) rather than treating this
                // as a partial packet and accumulating corrupt bytes.
