...
class Connection {
    ...
    _bind_events(host, port, callback) {
        ...
        self._socket.on('data', function (data) {
            ...
            while (xdr.pos < xdr.buffer.length) {
                ...
                var pendingOp = self._pending[0];
                if (process.env.DEBUG_FIREBIRD) {
                    console.log('[DEBUG] Processing response for operation:', pendingOp, 'queue length:', self._queue.length);
                }
    ...
    sendOpContAuth(authData, authDataEnc, pluginName) {
        ...
        // Convert hex string authData to Buffer if it's a string
        var authDataBuffer;
        if (typeof authData === 'string') {
            // authData is hex string representation, convert to Buffer
            authDataBuffer = Buffer.from(authData, 'hex');
        } else if (Buffer.isBuffer(authData)) {
            authDataBuffer = authData;
        } else {
            authDataBuffer = Buffer.alloc(0);
        }
        ...
        // Send null buffer for p_keys (length=0)
        msg.addInt(0);
    ...
    connect(options, callback) {
        ...
        function cb(err, ret) {
            ...
            // Debug logging for protocol negotiation
            if (process.env.DEBUG_FIREBIRD) {
                console.log('[DEBUG] Protocol negotiated:', ret.protocolVersion, 'arch:', ret.protocolArchitecture, 'min type:', ret.protocolMinimumType);
            }
...
function getOpcodeName(opcode) {
    const opcodeNames = {
        [Const.op_void]: 'op_void',
        [Const.op_connect]: 'op_connect',
        [Const.op_accept]: 'op_accept',
        [Const.op_response]: 'op_response',
        [Const.op_cond_accept]: 'op_cond_accept',
        [Const.op_accept_data]: 'op_accept_data',
        [Const.op_cont_auth]: 'op_cont_auth',
        [Const.op_crypt]: 'op_crypt',
        [Const.op_crypt_key_callback]: 'op_crypt_key_callback',
        [Const.op_fetch_response]: 'op_fetch_response',
        [Const.op_sql_response]: 'op_sql_response'
    };
...
function decodeResponse(data, callback, cnx, lowercase_keys, cb) {
    // Debug logging for connection issues
    if (process.env.DEBUG_FIREBIRD) {
        console.log('[DEBUG] decodeResponse called, buffer length:', data.buffer.length, 'pos:', data.pos);
    }
    ...
    try {
        ...
        // Debug logging for response opcode
        if (process.env.DEBUG_FIREBIRD) {
            console.log('[DEBUG] Response opcode:', r, '(0x' + r.toString(16) + ')', getOpcodeName(r));
        }
        ...
        switch (r) {
            ...
            case Const.op_accept_data:
                ...
                if (r === Const.op_cond_accept || r === Const.op_accept_data) {
                    ...
                    var keys = data.readArray(); // keys - read as array/buffer, not string!
            ...
            case Const.op_cont_auth:
                ...
                data.readArray(); // plist - read as buffer/array, not string
                data.readArray(); // pkeys - read as buffer/array, not string

                // Use _pendingAccept during auth flow, as accept is not set yet
                ...
                if (!cnx.options.pluginName) {
                    if (acceptObj.pluginName === pluginName) {
                        ...
                        return cb(new Error("Unable to connect with plugin " + acceptObj.pluginName));
                    }

                    if (pluginName === Const.AUTH_PLUGIN_LEGACY) { // Fallback to LegacyAuth
                        acceptObj.pluginName = pluginName;
                        acceptObj.authData = crypt.crypt(cnx.options.password, Const.LEGACY_AUTH_SALT).substring(2);
