...
const ServiceManager = function(connection) {
    ...
    this._charset = connection.options.charset || Const.DEFAULT_ENCODING
    connection.svc = this;
}
...
ServiceManager.prototype._createOutputStream = function (optread, buffersize, callback) {
    ...
    t.__proto__._read = function () {
        ...
        fct.call(self, { buffersize: buffersize }, function (err, data) {
            if (err) {
                selfread.push(err.message, this._charset);
                return;
            }
            if (data.line && data.line.length)
                selfread.push(data.line, this._charset);
            else
                selfread.push(null);
        });
    }
...
ServiceManager.prototype.backup = function (options, callback) {
    ...
    blr.addString2(Const.isc_spb_dbname, dbpath, this._charset);
    for (var i = 0; i < bckfiles.length; i++) {
        blr.addString2(Const.isc_spb_bkp_file, bckfiles[i].filename, this._charset);
        if (i !== bckfiles.length - 1) // not the end, so we need to write the size of this part (gsplit)
            blr.addString2(Const.isc_spb_bkp_length, bckfiles[i].sizefile, this._charset);
    }
    if (factor)
...
ServiceManager.prototype.nbackup = function (options, callback) {
    ...
    blr.addString2(Const.isc_spb_dbname, dbpath, this._charset);
    blr.addString2(Const.isc_spb_nbk_file, bckfile, this._charset);
    blr.addByteInt32(Const.isc_spb_nbk_level, level);
    blr.addString2(Const.isc_spb_nbk_direct, direct, this._charset);
    var opts = 0;
...
ServiceManager.prototype.restore = function(options, callback) {
    ...
    for (var i = 0; i < bckfiles.length; i++) {
        blr.addString2(Const.isc_spb_bkp_file, bckfiles[i], this._charset);
    }
    blr.addString2(Const.isc_spb_dbname, dbfile, this._charset);
    ...
    if (fixfssdata) blr.addString2(Const.isc_spb_res_fix_fss_data, fixfssdata, this._charset);
    if (fixfssmetadata) blr.addString2(Const.isc_spb_res_fix_fss_metadata, fixfssmetadata, this._charset);
    var opts = 0;
...
ServiceManager.prototype.nrestore = function (options, callback) {
    ...
    for (var i = 0; i < bckfiles.length; i++) {
        blr.addString2(Const.isc_spb_nbk_file, bckfiles[i], this._charset);
    }
    blr.addString2(Const.isc_spb_dbname, dbpath, this._charset);
    var self = this;
    this.connection.svcstart(blr, function (err, data) {
...
ServiceManager.prototype.getStats = function (options, callback) {
    ...
    if (opts)
        blr.addByteInt32(Const.isc_spb_options, opts);
    if (objects) blr.addString2(Const.isc_spb_command_line, objects, this._charset);
    var self = this;
    this.connection.svcstart(blr, function (err, data) {
        if (err) {
            doError(new Error(err), callback);
...
ServiceManager.prototype.getUsers = function (username, callback) {
    ...
    if (username) blr.addString2(Const.isc_spb_sec_username, username, this._charset);
    this.connection.svcstart(blr, function (err, data) {
        if (err) {
            doError(new Error(err), callback);
            return;
...
ServiceManager.prototype.addUser = function (username, password, options, callback) {
    ...
    blr.addString2(Const.isc_spb_sec_username, username, this._charset);
    blr.addString2(Const.isc_spb_sec_password, password, this._charset);
    if (rolename) blr.addString2(Const.isc_dpb_sql_role_name, rolename, this._charset);
    if (groupname) blr.addString2(Const.isc_spb_sec_groupname, groupname, this._charset);
    if (firsname) blr.addString2(Const.isc_spb_sec_firstname, firsname, this._charset);
    if (middlename) blr.addString2(Const.isc_spb_sec_middlename, middlename, this._charset);
    if (lastname) blr.addString2(Const.isc_spb_sec_lastname, lastname, this._charset);
...
ServiceManager.prototype.editUser = function (username, options, callback) {
    ...
    blr.addString2(Const.isc_spb_sec_username, username, this._charset);
    if (password) blr.addString2(Const.isc_spb_sec_password, password, this._charset);
    if (rolename) blr.addString2(Const.isc_dpb_sql_role_name, rolename, this._charset);
    if (groupname) blr.addString2(Const.isc_spb_sec_groupname, groupname, this._charset);
    if (firsname) blr.addString2(Const.isc_spb_sec_firstname, firsname, this._charset);
    if (middlename) blr.addString2(Const.isc_spb_sec_middlename, middlename, this._charset);
...
ServiceManager.prototype.startTrace = function (options, callback) {
    ...
    blr.addString2(Const.isc_spb_trc_cfg, configfile, this._charset);
    blr.addString2(Const.isc_spb_trc_name, tracename, this._charset);
    this.connection.svcstart(blr, function (err, data) {
        if (err) {
            doError(new Error(err), callback);
            return;
