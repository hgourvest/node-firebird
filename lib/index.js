...
ServiceManager.prototype.backup = function (options, callback) {
    ...
    var opts = 0;
    if (ignorechecksums) {
        opts = opts | isc_spb_bkp_ignore_checksums;
    }
    if (ignorelimbo) {
        opts = opts | isc_spb_bkp_ignore_limbo;
    }
    if (metadataonly) {
        opts = opts | isc_spb_bkp_metadata_only;
    }
    if (nogarbagecollect) {
        opts = opts | isc_spb_bkp_no_garbage_collect;
    }
    if (olddescriptions) {
        opts = opts | isc_spb_bkp_old_descriptions;
    }
    if (nontransportable) {
        opts = opts | isc_spb_bkp_non_transportable;
    }
    if (convert) {
        opts = opts | isc_spb_bkp_convert;
    }
    if (expand) {
        opts = opts | isc_spb_bkp_expand;
    }
    if (notriggers) {
...
ServiceManager.prototype.nbackup = function (options, callback) {
    ...
    var opts = 0;
    if (notriggers) {
        opts = opts | isc_spb_nbk_no_triggers;
    }
    blr.addByteInt32(isc_spb_options, opts);
    var self = this;
    this.connection.svcstart(blr, function (err, data) {
...
ServiceManager.prototype.restore = function (options, callback) {
    ...
    if (fixfssdata) {
        blr.addString2(isc_spb_res_fix_fss_data, fixfssdata, DEFAULT_ENCODING);
    }
    if (fixfssmetadata) {
        blr.addString2(isc_spb_res_fix_fss_metadata, fixfssmetadata, DEFAULT_ENCODING);
    }
    var opts = 0;
    if (deactivateindexes) {
        opts = opts | isc_spb_res_deactivate_idx;
    }
    if (noshadow) {
        opts = opts | isc_spb_res_no_shadow;
    }
    if (novalidity) {
        opts = opts | isc_spb_res_no_validity;
    }
    if (individualcommit) {
        opts = opts | isc_spb_res_one_at_a_time;
    }
    if (replace) {
        opts = opts | isc_spb_res_replace;
    }
    if (create) {
        opts = opts | isc_spb_res_create;
    }
    if (useallspace) {
        opts = opts | isc_spb_res_use_all_space;
    }
    if (metadataonly) {
        opts = opts | isc_spb_res_fix_fss_metadata;
    }
    if (opts) {
        blr.addByteInt32(isc_spb_options, opts);
    }
...
ServiceManager.prototype._fixpropertie = function (options, callback) {
    ...
    blr.addString2(isc_spb_dbname, dbpath, DEFAULT_ENCODING);
    if (dialect) {
        blr.addByteInt32(isc_spb_prp_set_sql_dialect, dialect);
    }
    if (sweep) {
        blr.addByteInt32(isc_spb_prp_sweep_interval, sweep);
    }
    if (pagebuffers) {
        blr.addByteInt32(isc_spb_prp_page_buffers, pagebuffers);
    }
    if (shutdown != null) {
    ...
    if (forcewrite) {
        blr.addBytes([isc_spb_prp_write_mode, isc_spb_prp_wm_sync]);
    }
    if (forcewrite != null && !forcewrite) {
        blr.addBytes([isc_spb_prp_write_mode, isc_spb_prp_wm_async]);
    }
    if (accessmode) {
        blr.addBytes([isc_spb_prp_access_mode, isc_spb_prp_am_readwrite]);
    }
    if (accessmode != null && !accessmode) {
        blr.addBytes([isc_spb_prp_access_mode, isc_spb_prp_am_readonly]);
    }
    if (reservespace) {
        blr.addBytes([isc_spb_prp_reserve_space, isc_spb_prp_res]);
    }
    if (reservespace != null && !reservespace) {
        blr.addBytes([isc_spb_prp_reserve_space, isc_spb_prp_res_use_full]);
    }
    var opts = 0;
    if (shadow) {
        opts = opts | isc_spb_prp_activate;
    }
    if (online) {
        opts = opts | isc_spb_prp_db_online;
    }
    if (opts) {
        blr.addByteInt32(isc_spb_options, opts);
    }
...
ServiceManager.prototype.validate = function (options, callback) {
    ...
    var opts = 0;
    if (checkdb) {
        opts = opts | isc_spb_rpr_check_db;
    }
    if (ignorechecksums) {
        opts = opts | isc_spb_rpr_ignore_checksum;
    }
    if (killshadows) {
        opts = opts | isc_spb_rpr_kill_shadows;
    }
    if (mend) {
        opts = opts | isc_spb_rpr_mend_db;
    }
    if (validate) {
        opts = opts | isc_spb_rpr_validate_db;
    }
    if (full) {
        opts = opts | isc_spb_rpr_full;
    }
    if (sweep) {
        opts = opts | isc_spb_rpr_sweep_db;
    }
    if (listlimbo) {
        opts = opts | isc_spb_rpr_list_limbo_trans;
    }
    if (icu) {
        opts = opts | isc_spb_rpr_icu;
    }
    blr.addByteInt32(isc_spb_options, opts);
    var self = this;
    this.connection.svcstart(blr, function (err, data) {
...
ServiceManager.prototype.getStats = function (options, callback) {
    ...
    var opts = 0;
    if (record) {
        opts = opts | isc_spb_sts_record_versions;
    }
    if (nocreation) {
        opts = opts | isc_spb_sts_nocreation;
    }
    if (tables) {
        opts = opts | isc_spb_sts_table;
    }
    if (pages) {
        opts = opts | isc_spb_sts_data_pages;
    }
    if (header) {
        opts = opts | isc_spb_sts_hdr_pages;
    }
    if (indexes) {
        opts = opts | isc_spb_sts_idx_pages;
    }
    if (tablesystem) {
        opts = opts | isc_spb_sts_sys_relations;
    }
    if (encryption) {
        opts = opts | isc_spb_sts_encryption;
    }
    if (opts) {
        blr.addByteInt32(isc_spb_options, opts);
    }
    if (objects) {
        blr.addString2(isc_spb_command_line, objects, DEFAULT_ENCODING);
    }
    var self = this;
    this.connection.svcstart(blr, function (err, data) {
        if (err) {
...
ServiceManager.prototype.addUser = function (username, password, options, callback) {
    ...
    if (rolename) {
        blr.addString2(isc_dpb_sql_role_name, rolename, DEFAULT_ENCODING);
    }
    if (groupname) {
        blr.addString2(isc_spb_sec_groupname, groupname, DEFAULT_ENCODING);
    }
    if (firsname) {
        blr.addString2(isc_spb_sec_firstname, firsname, DEFAULT_ENCODING);
    }
    if (middlename) {
        blr.addString2(isc_spb_sec_middlename, middlename, DEFAULT_ENCODING);
    }
    if (lastname) {
        blr.addString2(isc_spb_sec_lastname, lastname, DEFAULT_ENCODING);
    }
    if (userid != null) {
        blr.addByteInt32(isc_spb_sec_userid, userid);
    }
    if (groupid != null) {
        blr.addByteInt32(isc_spb_sec_groupid, groupid);
    }
    if (admin != null) {
        blr.addByteInt32(isc_spb_sec_admin, admin);
    }
...
exports.Connection.prototype._bind_events = function (host, port, callback) {
    ...
    self._socket.on('data', function (data) {
        ...
        while (xdr.pos < xdr.buffer.length) {
            ...
            decodeResponse(xdr, cb, self.db, self._lowercase_keys, function (err, obj) {
                ...
                while (!finishDescribe) {
                    switch (br.readByteCode()) {
                        ...
                        case isc_info_sql_type:
                            ...
                            switch (type & ~1) {
                                case SQL_VARYING:
                                    param = new SQLVarString();
                                    break;
                                case SQL_NULL:
                                    param = new SQLVarNull();
                                    break;
                                case SQL_TEXT:
                                    param = new SQLVarText();
                                    break;
                                case SQL_DOUBLE:
                                    param = new SQLVarDouble();
                                    break;
                                case SQL_FLOAT:
                                case SQL_D_FLOAT:
                                    param = new SQLVarFloat();
                                    break;
                                case SQL_TYPE_DATE:
                                    param = new SQLVarDate();
                                    break;
                                case SQL_TYPE_TIME:
                                    param = new SQLVarTime();
                                    break;
                                case SQL_TIMESTAMP:
                                    param = new SQLVarTimeStamp();
                                    break;
                                case SQL_BLOB:
                                    param = new SQLVarBlob();
                                    break;
                                case SQL_ARRAY:
                                    param = new SQLVarArray();
                                    break;
                                case SQL_QUAD:
                                    param = new SQLVarQuad();
                                    break;
                                case SQL_LONG:
                                    param = new SQLVarInt();
                                    break;
                                case SQL_SHORT:
                                    param = new SQLVarShort();
                                    break;
                                case SQL_INT64:
                                    param = new SQLVarInt64();
                                    break;
                                case SQL_BOOLEAN:
                                    param = new SQLVarBoolean();
                                    break;
                                default:
