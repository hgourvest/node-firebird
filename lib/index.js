    ...
    self._socket.on('close', function() {
        ...
        setImmediate(function() {
            ...
            var ctx = new Connection(host, port, function(err) {
                //
                // Fix Florent : s'il y a un attach de fait sur une base par le biais d'un serveur Firebird et que
                // le service de celui ci est coupé pour X ou Y raison, le reconnect automatique de node-firebird
                // passe ici et donc génére des erreurs en continu et tente un connect a chaque fois alors que le
                // serveur lui même ne répond pas.
                //
                if (err) return;
                ...
                switch (meta.type) {
                    ...
                    case SQL_TYPE_TIME:
                        ...
                        if (value instanceof Date) {
                            ...
                            if (/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/g.test(value) || /^[0-9]{2}:[0-9]{2}$/g.test(value)) // test le format 0000-00-00
                            {
