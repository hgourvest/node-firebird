...
const
    ...
    DEFAULT_TIMEOUT = 10000; // 5 seconds for socket
...
Statement.prototype.execute = function(transaction, params, callback, asObject) {
    ...
    this.connection.executeStatement(transaction, this, params, callback, asObject);
};
...
Transaction.prototype.execute = function(query, params, callback, asObject) {
...
Transaction.prototype.query = function(query, params, callback) {
    ...
    this.execute(query, params, mapFieldNames(callback, this.db));
};
...
Database.prototype.detach = function(callback) {
    var self = this;
    this.connection.detach(function(err, obj) {
...
Database.prototype.execute = function(query, params, callback, asObject) {
    ...
    this.connection.startTransaction(function(err, transaction) {
...
function mapFieldNames(callback, db) {
    ...
    return function(err, result, meta, isSelect) {
        ...
        for (var k = 0, length = meta.length; k < length; k++)
            lower[k] = meta[k].alias.toLowerCase();

        function convert(value, index) {
            var obj = {};
            for (var j = 0, length = lower.length; j < lower.length; j++)
                obj[lower[j]] = value[j];
            db.emit('row', obj, i);
            return obj;
        }
        ...
        if (!isSelect) {
            var a = convert(result);
            db.emit('result', a);
            callback(undefined, a, meta, isSelect);
            return;
        }
...
Database.prototype.query = function(query, params, callback) {
    ...
    this.execute(query, params, mapFieldNames(callback, this), true);
};
...
exports.attach = function(options, callback){
    ...
    var cnx = this.connection = new Connection(host, port, function(err) {
        ...
        cnx.connect(options.database, function(err) {
            if (err)
                doError(err, callback);
            else
...
var Connection = exports.Connection = function (host, port, callback, timeout) {
    ...
    this._timeoutDelay = timeout || DEFAULT_TIMEOUT;
...
Connection.prototype.executeStatement = function(transaction, statement, params, callback, asObject) {
...
function fetchBlobs(statement, transaction, rows, callback) {
            ...
            function read() {
                statement.connection.getSegment(blob, function(err, ret) {
                    ...
                    if (ret.handle === 2) { // ??? TODO: === or ==
                        ...
                        if (statement.output[col].subType === isc_blob_text) { // TODO: === or ==
                            if (buffer)
    ...
    for (var r = 0, length = rows.data.length; r < length; r++) {
        for (var c = 0, sublength = indexes.length; c < sublength; c++) {
            fetch(r, indexes[c], function(err) {
                if (!err) {
                    count--;
                    if (count === 0)
                       callback(undefined, rows)
