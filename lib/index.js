...
exports.Connection.prototype._bind_events = function(host, port, callback) {
    ...
    self._socket.on('data', function(data) {
        ...
        while (xdr.pos < xdr.buffer.length) {
            ...
            try {
                cb = self._queue[0];
                obj = decodeResponse(xdr, cb, self, self._lowercase_keys);
...
function decodeResponse(data, callback, cnx, lowercase_keys) {
	do {
    ...
    switch (r) {
		case op_response:
			...
			} while (callback && callback.lazy_count && callback.lazy_count > 0);

			return response;
        case op_fetch_response:
		case op_sql_response:
			return parseOpFetchResponse(data, callback.statement, lowercase_keys, r);
		case op_accept:
		case op_cond_accept:
		case op_accept_data:
			return parseOpConnectResponse(r, data, cnx);
        default:
            throw new Error('Unexpected:' + r);
    }
}

function parseOpResponse(data, response) {
	...
	throw new Error('Unexpected end of op response');
}

function parseOpConnectResponse(r, data, cnx) {
	var accept = {
	...
	if (r === op_cond_accept || r === op_accept_data) {
		...
		if (is_authenticated === 0) {
			if (accept.pluginName === AUTH_PLUGIN_LEGACY) {
				...
				throw new Error('Unknow auth plugin : ' + accept.pluginName);
			}
		} else {
			accept.authData = '';
			accept.sessionKey = '';
...
function parseOpFetchResponse(data, statement, lowercase_keys, r) {
	...
	while (data.fcount && (data.fstatus !== 100)) {
		...
		if (lowerV13) {
			for (length = output.length; data.fcolumn < length; data.fcolumn++) {
				...
				decodeColumn(r, data, statement, item, custom, lowerV13);
			}
		} else {
	...
	return { data: data.frows, fetched: Boolean(!isOpFetch|| data.fstatus === 100) };
}

function decodeColumn(r, data, statement, item, custom, lowerV13) {
	var _xdrpos;
