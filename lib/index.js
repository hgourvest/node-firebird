        ...
        exports.attach(self.options, function (err, db) {
            if (!err) {
                ...
                _db.on('detach', function () {
                    ...
                    if (_db.connection._isClosed || _db.connection._isDetach || _db.connection._pooled == false)
    ...
    switch (r) {
        ...
        case op_fetch_response:
            ...
            while (count && (status !== 100)) {
                ...
                status = data.readInt();
                count = data.readInt();
                // don't resend previous row
                if (_fetchrow >= data.index) {
                    statement.connection.db.emit('row', row, data.index, custom.asObject);

                    if (!custom.asStream)
                        rows.push(row);

                    if (custom.on)
                        custom.on(row, data.index);
                    data.index++;
                }
                _fetchrow++;
            }
